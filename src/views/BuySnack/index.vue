<template>
    <div class="container">
        <div class="head">
            <div class="title">零食购买</div>
            <div @click="router.push('/')" class="goback">首页</div>
            <div class="search">
                <a-input-search v-model:value="value" placeholder="输入零食的名字" enter-button @search="SeachFn" />
            </div>
        </div>
        <div class="content">
            <div class="top"><svg t="1716980831260" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="3623" width="64" height="64">
                    <path
                        d="M685.2 820.5l-11.6-2.5c-5.4-1.2-8.9-6.5-7.7-11.9l45.2-210.4 31.2 6.7-45.2 210.4c-1.1 5.4-6.5 8.9-11.9 7.7z"
                        fill="#FFFFFF" p-id="3624"></path>
                    <path
                        ...
                        fill="#232323" p-id="3625"></path>
                    <path d="M76.6 244.4h463.5v533.8H76.6z" fill="#FFE092" p-id="3626"></path>
                    <path
                    ...
                    <path
                        ...
                        fill="#232323" p-id="3634"></path>
                    <path d="M296.4 707.9s-32.4-93.7 26.2-133.1S468 583 468 583s-115.5 38.7-171.6 124.9z" fill="#EE9426"
                        p-id="3635"></path>
                    <path
                        ...
                        fill="#232323" p-id="3636"></path>
                    <path d="M766.3 424m-179.5 0a179.5 179.5 0 1 0 359 0 179.5 179.5 0 1 0-359 0Z" fill="#FFD2D2"
                        p-id="3637"></path>
                    <path
                    ...
                    <path
                        ...
                        fill="#232323" p-id="3640"></path>
                </svg>
                <p class="title">零食列表({{ snackList.length }}种)</p>
            </div>
            <div class="bottom">
                <Card v-for="snack in snackList" :key="snack.ID" :info="snack" class="card" @buySuccess="buySuccess" />
            </div>
        </div>
    </div>
</template>
...
import { getSnack, searchSnack } from '@/apis/snack/index'
...
onMounted(async () => {
    const res = await getSnack()
    snackList.value = res.data.data

})

const buySuccess = async (payload: Boolean) => {
    const res = await getSnack()
    snackList.value = res.data.data
}
...
const SeachFn = async () => {
    ...
    if (value.value === '') {
        const All = await getSnack()
        snackList.value = All.data.data
        return
    }
    const res = await searchSnack(value.value)
    snackList.value = res.data.data
...
.container {
    ...
    .head {
        ...
        .title {
            ...
            line-height: 100px;
            font-family: "也字工厂";
            background-image: url("@/assets/images/猫眼.png");
